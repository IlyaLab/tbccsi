#!/usr/bin/env python3
"""
Baseline Linear Probe for Macrophage Count Regression (ResNet backbone)

Two-phase workflow (identical interface to domain_specific_frozen_codex_7.py):
  Phase 1 (--mode extract): Load pre-augmented tile PNGs → frozen ResNet
           → save pooled feature embeddings to disk
  Phase 2 (--mode train):   Load cached embeddings → train single linear
           regression head

Architecture:
  - Input: ResNet global-average-pooled features (2048-d for ResNet-50)
  - Single linear layer → 2 outputs: log1p(n_m1), log1p(n_m2)

No domain-specific layers, no CODEX features, no weighted sampling.
Used as a comparison baseline against domain_specific_frozen_codex_7.py.
...
class BaselineRegressor(nn.Module):
    """Single linear layer: embedding_dim -> 2 outputs (log1p n_m1, log1p n_m2)."""
    ...
    def forward(self, embeddings: torch.Tensor) -> torch.Tensor:
        return self.head(embeddings)  # [B, 2]: [log1p_n_m1, log1p_n_m2]
...
class EmbeddingDataset(Dataset):
    """Loads precomputed embeddings for baseline training (no CODEX features).
...
# ---------------------------------------------------------------------------
# Embedding extraction (Phase 1) — identical to full model
# ---------------------------------------------------------------------------
def extract_embeddings(
    ...
    """Extract ResNet global-average-pooled features from pre-augmented tile PNGs.

    Uses timm with num_classes=0 to obtain the pooled feature vector
    (2048-d for ResNet-50, 512-d for ResNet-18/34).
    """
    ...
    for batch_imgs, batch_paths in loader:
        ...
        with torch.no_grad(), autocast():
            features = backbone(batch_imgs)  # [B, embedding_dim]

        for path, emb in zip(batch_paths, features.cpu()):
            embeddings[path] = emb
...
def train_one_epoch(
    ...
    for batch_idx, (embs, targets, _domains) in enumerate(loader):
...
# ---------------------------------------------------------------------------
# Plotting & prediction output (identical to full model)
# ---------------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(
        description="Baseline Linear Probe for Macrophage Count Regression (ResNet backbone)"
    )
    ...
    extract_parser.add_argument("--backbone", type=str, default="resnet50")
    ...
    train_parser = subparsers.add_parser(
        "train", help="Train baseline linear probe on precomputed embeddings"
    )
    ...
    # Architecture
    train_parser.add_argument("--embedding_dim", type=int, default=2048)
    ...
    for train_df, val_df, fold in fold_splits:
        ...
        model = BaselineRegressor(embedding_dim=args.embedding_dim).to(device)
    ...
    # LOSO held-out specimen evaluation
    loso_test_results = {}
    if len(loso_test_df) > 0 and best_state_dict is not None:
        logger.info(f"\nLOSO test evaluation on {len(loso_test_df)} held-out tiles...")
        ...
        logger.info(f"LOSO test predictions ({len(loso_pred_df)} samples) → loso_test_predictions.csv")
        loso_test_results = {
        ...
        logger.info(
            f"LOSO test: r_{args.target1}={loso_metrics['pearson_t1']:.4f} "
            f"r_{args.target2}={loso_metrics['pearson_t2']:.4f} "
            f"r_mean={loso_metrics['pearson_mean']:.4f}"
        )
