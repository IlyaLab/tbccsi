#!/usr/bin/env python3
"""
Shared MLP Probe for Macrophage Count Regression
...
Two-phase workflow (identical interface to domain_specific_frozen_codex_7.py):
  ...
  Phase 2 (--mode train):   Load cached embeddings → train shared 2-layer MLP
           regression head

Architecture:
  - Input: Virchow2 CLS embeddings (1280-d)
  - Shared MLP: embedding_dim → fc1_dim → head_dim → 2 outputs
  - BN + GELU + Dropout at each hidden layer

No domain-specific layers, no CODEX features.
Sits between baseline_virchow2.py (linear probe) and the full domain-specific
model — isolates the contribution of the MLP head depth from domain routing.
Used as a comparison model against domain_specific_frozen_codex_7.py.
...
class BaselineRegressor(nn.Module):
    """
    Shared 2-layer MLP head: embedding_dim → fc1_dim → head_dim → 2 outputs.

    Mirrors the structure of the full model's domain branch + shared head,
    but without domain-specific routing.
    """
    ...
    def forward(self, embeddings: torch.Tensor) -> torch.Tensor:
        return self.net(embeddings)  # [B, 2]: [log1p_n_m1, log1p_n_m2]
