#!/usr/bin/env python3
"""
Domain-Specific Regressor with CODEX Feature Integration

Two-phase workflow:
  Phase 1 (--mode extract): Load pre-augmented tile PNGs → frozen Virchow2
           → save CLS embeddings to disk
  Phase 2 (--mode train):   Load cached embeddings + CODEX features → train
           domain-specific dual-output regression head

Architecture:
  - Input: Virchow2 CLS embeddings (1280-d)
  - Domain-specific 2-layer MLP branches (nonlinear remap per domain/stain)
  - Domain-specific batch normalization throughout
  - Enhanced dropout: global + domain-specific dropout rates
  - CODEX protein expression features: fused after domain branches
  - Shared regression head → 2 outputs: log1p(n_m1), log1p(n_m2)

Targets: log1p-transformed macrophage counts per tile.
  - Predictions recovered via expm1(output).clip(0)
  - m1_ratio = pred_m1 / (pred_m1 + pred_m2)  when (pred_m1 + pred_m2) > 0
...
class DomainSpecificBatchNorm1d(nn.Module):
    """Maintains separate BN statistics per domain."""

    def __init__(self, num_features: int, num_domains: int, **kwargs):
        super().__init__()
        self.num_domains = num_domains
        self.bns = nn.ModuleList(
            [nn.BatchNorm1d(num_features, **kwargs) for _ in range(num_domains)]
        )

    def forward(self, x: torch.Tensor, domain_idx: int) -> torch.Tensor:
        if x.shape[0] == 1 and self.training:
            return x  # skip BN for single-sample batches
        return self.bns[domain_idx](x)


# ---------------------------------------------------------------------------
# Domain-specific dropout
# ---------------------------------------------------------------------------
class DomainSpecificDropout(nn.Module):
    """Applies different dropout rates per domain.

    Some domains (e.g. IHC stains with more noise) may benefit from heavier
    regularization than cleaner H&E embeddings.
    """

    def __init__(self, base_rate: float, domain_rates: Optional[Dict[int, float]] = None):
        super().__init__()
        self.base_rate = base_rate
        self.domain_rates = domain_rates or {}

    def forward(self, x: torch.Tensor, domain_idx: int) -> torch.Tensor:
        rate = self.domain_rates.get(domain_idx, self.base_rate)
        return F.dropout(x, p=rate, training=self.training)


# ---------------------------------------------------------------------------
# CODEX feature encoder
# ---------------------------------------------------------------------------
class CodexFeatureEncoder(nn.Module):
    """Encodes CODEX protein expression features into a fixed-dim embedding."""
...
# Main classifier
# ---------------------------------------------------------------------------
class DomainSpecificRegressor(nn.Module):
    """
    Precomputed embeddings → 2-layer domain-specific branches → CODEX fusion
    → shared regression head → 2 continuous outputs (log1p n_m1, log1p n_m2).

    Architecture flow:
        precomputed embedding (1280-d)
                    ↓
        domain-specific 2-layer MLP (one per domain)
          FC1: 1280 → fc1_dim  + BN + GELU + Dropout
          FC2: fc1_dim → fc1_dim + BN + GELU + Dropout
                    ↓
              branch output (fc1_dim)
                    ↓
        ┌───────────┴───────────┐
        │    (if CODEX avail.)   │
        │  CODEX features → enc  │
        ...
        └───────────┬───────────┘
                    ↓
          shared head (with BN, dropout)
                    ↓
          2 regression outputs: [log1p_n_m1, log1p_n_m2]
    """

    def __init__(
        self,
        output_dim: int = 2,       # [log1p_n_m1, log1p_n_m2]
        num_domains: int = 2,
        embedding_dim: int = 1280,
        fc1_dim: int = 512,
        head_dim: int = 256,
        codex_input_dim: Optional[int] = None,
        codex_hidden_dim: int = 128,
        codex_embed_dim: int = 128,
        base_dropout: float = 0.4,
        domain_dropout_rates: Optional[Dict[int, float]] = None,
        codex_dropout: float = 0.3,
        ...
        # --- Domain-specific 2-layer MLP branches ---
        # Layer 1: project from embedding dim to fc1_dim (dimensionality reduction)
        self.domain_fc1 = nn.ModuleList([
            nn.Linear(embedding_dim, fc1_dim) for _ in range(num_domains)
        ])
        self.domain_bn1 = DomainSpecificBatchNorm1d(fc1_dim, num_domains)
        self.domain_dropout1 = DomainSpecificDropout(
            base_rate=base_dropout,
            domain_rates=domain_dropout_rates,
        )
        # Layer 2: same-dim nonlinear transform (learns domain-specific warping)
        self.domain_fc1b = nn.ModuleList([
            nn.Linear(fc1_dim, fc1_dim) for _ in range(num_domains)
        ])
        self.domain_bn1b = DomainSpecificBatchNorm1d(fc1_dim, num_domains)
        self.domain_dropout1b = DomainSpecificDropout(
            base_rate=base_dropout,
            domain_rates=domain_dropout_rates,
        )

        # --- CODEX feature encoder + fusion ---
        if codex_input_dim is not None and codex_input_dim > 0:
            self.codex_encoder = CodexFeatureEncoder(
                input_dim=codex_input_dim,
                hidden_dim=codex_hidden_dim,
                output_dim=codex_embed_dim,
                dropout=codex_dropout,
            )
            fusion_input_dim = fc1_dim + codex_embed_dim
            self.fusion_fc = nn.Sequential(
                nn.Linear(fusion_input_dim, fc1_dim),
        ...
        self.head_bn = DomainSpecificBatchNorm1d(head_dim, num_domains)
        self.head_dropout = DomainSpecificDropout(
            base_rate=head_dropout,
            domain_rates=None,
        )
        # Output: [log1p_n_m1, log1p_n_m2] — no activation (raw regression)
        self.regressor = nn.Linear(head_dim, output_dim)

        # Initialize weights
        self._init_weights()

    def _init_weights(self):
        for module_list in [self.domain_fc1, self.domain_fc1b]:
            for m in module_list:
                nn.init.kaiming_normal_(m.weight, nonlinearity="linear")
                if m.bias is not None:
                    nn.init.zeros_(m.bias)
        for m in [self.head_fc, self.regressor]:
            nn.init.kaiming_normal_(m.weight, nonlinearity="linear")
            if m.bias is not None:
                nn.init.zeros_(m.bias)
    ...
    def forward(
        ...
        domain_idx: int,
        codex_features: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        # --- Domain-specific 2-layer MLP ---
        # Layer 1: project from embedding space to fc1_dim
        x = self.domain_fc1[domain_idx](embeddings)
        x = self.domain_bn1(x, domain_idx)
        x = F.gelu(x)
        x = self.domain_dropout1(x, domain_idx)
        # Layer 2: nonlinear domain-specific transform
        x = self.domain_fc1b[domain_idx](x)
        x = self.domain_bn1b(x, domain_idx)
        x = F.gelu(x)
        x = self.domain_dropout1b(x, domain_idx)

        # --- CODEX fusion ---
        if self.has_codex and codex_features is not None:
            codex_emb = self.codex_encoder(codex_features)
            x = torch.cat([x, codex_emb], dim=-1)
            x = self.fusion_fc(x)

        # --- Shared regression head ---
        x = self.head_fc(x)
        x = self.head_bn(x, domain_idx)
        x = F.gelu(x)
        x = self.head_dropout(x, domain_idx)
        return self.regressor(x)  # [B, 2]: [log1p_n_m1, log1p_n_m2]
...
class EmbeddingDataset(Dataset):
    """Loads precomputed embeddings + optional CODEX features.

    Expects:
        - df: DataFrame with label, domain, slide_id, and optional codex_* columns
        - embeddings: dict mapping path_col value → embedding tensor
        - codex_cols: list of CODEX feature column names
    """
    ...
    def __init__(
        ...
        codex_cols: Optional[List[str]] = None,
    ...
    def __getitem__(self, idx):
        ...
        # Regression targets: log1p-transformed counts
        ...
        if self.codex_cols:
            codex = torch.tensor(
                row[self.codex_cols].values.astype(np.float32),
                dtype=torch.float32,
            )
        else:
            codex = torch.tensor([], dtype=torch.float32)

        return emb, targets, domain, codex


def collate_fn(batch):
    embs, targets, domains, codex = zip(*batch)
    embs = torch.stack(embs)
    targets = torch.stack(targets)          # [B, 2] float
    domains = torch.tensor(domains, dtype=torch.long)
    if codex[0].numel() > 0:
        codex = torch.stack(codex)
    else:
        codex = None
    return embs, targets, domains, codex
...
def load_embeddings(embeddings_path: str) -> Dict[str, torch.Tensor]:
    ...
    Supports:
        - Single .pt file: dict mapping tile_path → embedding tensor
        - Directory of .pt files: each file is {tile_path_stem}.pt containing
          a single embedding tensor; or a dict of embeddings
    """
...
def extract_embeddings(
    ...
    Reads tile paths from the CSV, runs them through the frozen backbone
    with only resize + normalize (no augmentation — tiles are already
    augmented), and saves a dict of {tile_path: embedding} to output_path.
    """
    ...
    # Minimal transform: just resize + normalize (tiles already augmented)
    ...
    # Simple image dataset (no labels needed)
    ...
    for batch_imgs, batch_paths in loader:
        ...
        with torch.no_grad(), autocast():
            ...
            # Handle different Virchow2 output formats
            if isinstance(output, tuple):
                cls_tokens = output[0]
            elif output.dim() == 3:
                cls_tokens = output[:, 0, :]  # [B, N, D] → CLS
            else:
                cls_tokens = output  # already pooled
...
def train_one_epoch(
    ...
    for batch_idx, (embs, targets, domains, codex) in enumerate(loader):
        embs    = embs.to(device, non_blocking=True)
        targets = targets.to(device, non_blocking=True)   # [B, 2] log1p
        domains_np = domains.numpy()
        if codex is not None:
            codex = codex.to(device, non_blocking=True)

        optimizer.zero_grad(set_to_none=True)

        unique_domains = np.unique(domains_np)
        total_loss  = torch.tensor(0.0, device=device)
        batch_preds = torch.zeros(len(targets), model.output_dim, device=device)
        ...
        with autocast():
            for d in unique_domains:
                ...
                d_preds = model(embs[idx], domain_idx=int(d),
                                codex_features=codex[idx] if codex is not None else None)
                d_loss  = criterion(d_preds, targets[idx_t])

                total_loss = total_loss + d_loss * len(idx)
                batch_preds[idx_t] = d_preds.detach().float()
        ...
        running_loss += total_loss.item() * len(targets)

        # Recover count-space predictions for metric tracking
...
def validate(
    ...
    for embs, targets, domains, codex in loader:
        embs    = embs.to(device, non_blocking=True)
        ...
        with autocast():
            for d in unique_domains:
                ...
                d_preds = model(embs[idx], domain_idx=int(d),
                                codex_features=codex[idx] if codex is not None else None)
                batch_preds[idx_t] = d_preds.float()
        ...
        all_domains.extend(domains_np)

    all_preds   = np.concatenate(all_preds,   axis=0)   # [N, 2] log1p space
    all_targets = np.concatenate(all_targets, axis=0)   # [N, 2] log1p space
    ...
    return {
        ...
        # Per-sample outputs for scatter plots
...
def plot_scatter_by_domain(
    ...
    for true_vals, pred_vals, ax, title, pkey in [
        ...
        for di, d in enumerate(unique_domains):
            ...
            r      = val_metrics["domain_metrics"][d][pkey]
...
def plot_combined_scatter(
    ...
    """Pooled true vs predicted scatter across all CV folds."""
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt

    combined = pd.concat(all_fold_predictions, ignore_index=True)
    domain_colors = ["tab:blue", "tab:orange", "tab:green", "tab:red", "tab:purple"]
    unique_domains = sorted(combined["domain"].unique())
...
def main():
    parser = argparse.ArgumentParser(
        description="Domain-Specific Regressor with CODEX (Two-Phase Workflow)"
    )
    ...
    # ---- Phase 2: Train regressor ----
    train_parser = subparsers.add_parser(
        "train", help="Train domain-specific regressor on precomputed embeddings"
    )
    ...
    train_parser.add_argument("--codex_prefix", type=str, default="codex_")
    ...
    train_parser.add_argument("--codex_hidden_dim", type=int, default=128)
    train_parser.add_argument("--codex_embed_dim", type=int, default=128)

    # Dropout
    train_parser.add_argument("--base_dropout", type=float, default=0.4)
    train_parser.add_argument("--head_dropout", type=float, default=0.5)
    train_parser.add_argument("--codex_dropout", type=float, default=0.3)
    train_parser.add_argument("--domain_dropout_json", type=str, default=None,
                        help='JSON dict mapping domain_idx→dropout, e.g. \'{"0":0.3,"1":0.5}\'')
    ...
    # Parse domain dropout rates
    domain_dropout_rates = None
    if args.domain_dropout_json:
        raw = json.loads(args.domain_dropout_json)
        domain_dropout_rates = {int(k): float(v) for k, v in raw.items()}
        logger.info(f"Domain-specific dropout rates: {domain_dropout_rates}")

    # Parse domain names for plot labels
    domain_names = None
    ...
    # Detect CODEX columns
    codex_cols = [c for c in df.columns if c.startswith(args.codex_prefix)]
    codex_input_dim = len(codex_cols) if codex_cols else None
    if codex_cols:
        logger.info(f"Found {len(codex_cols)} CODEX features: {codex_cols[:5]}...")
    else:
        logger.info("No CODEX features found — embedding-only mode")
    ...
    if args.split_col and args.split_col in df.columns:
        # Use pre-defined spatial split
        logger.info(f"Using pre-defined split from column '{args.split_col}'")
        splits = df[args.split_col].unique()
        logger.info(f"Split values: {sorted(splits)}")
        ...
        fold_splits = [(train_df, val_df, 0)]  # single "fold"
        n_folds_actual = 1
    else:
        loso_test_df = pd.DataFrame()
        # GroupKFold on slide_id (no label stratification needed for regression)
    ...
    for train_df, val_df, fold in fold_splits:
        ...
        model = DomainSpecificRegressor(
            ...
            codex_input_dim=codex_input_dim,
            codex_hidden_dim=args.codex_hidden_dim,
            codex_embed_dim=args.codex_embed_dim,
            base_dropout=args.base_dropout,
            domain_dropout_rates=domain_dropout_rates,
            codex_dropout=args.codex_dropout,
        ...
        # MSELoss on log1p-transformed targets
        ...
        train_ds = EmbeddingDataset(train_df, embeddings, codex_cols or None, path_col=args.path_col,
                                    target1=args.target1, target2=args.target2)
        val_ds   = EmbeddingDataset(val_df,   embeddings, codex_cols or None, path_col=args.path_col,
                                    target1=args.target1, target2=args.target2)
    ...
    if len(loso_test_df) > 0 and best_state_dict is not None:
        ...
        loso_test_ds = EmbeddingDataset(loso_test_df, embeddings, codex_cols or None,
                                        path_col=args.path_col,
                                        target1=args.target1, target2=args.target2)
